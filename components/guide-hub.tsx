  ...
  const results = useMemo(
    () =>
      clusters
        ...
        .filter(
          (a) =>
            (category === 'Tümü' || a.category === category) &&
  ...
  return (
    <>
      <div className="guide-controls">
        ...
        <div className="category-filters">
          ...
          {clusters.map((c) => (
            ...
            >
              {c.category} <span>{c.articles.length}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <section className="all-guides">
        ...
        {category === 'Tümü' && !query ? (
          clusters.map((c) => (
            <div className="guide-group" key={c.category}>
              <h3>
                {c.category} <span>{c.articles.length} yazı</span>
              </h3>
              <div>
                {c.articles.map((a) => (
                  <Link
                    key={a.path}
                    href={a.path}
                    data-content-status={a.contentStatus}
                  >
                    {a.isPillar && <small>Ana rehber</small>}
